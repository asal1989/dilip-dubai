...
- **Real domain**: `src/app/layout.tsx`, `robots.ts` and `sitemap.ts` read the site's public URL from the `NEXT_PUBLIC_SITE_URL` environment variable, falling back to `https://example.com` if it isn't set. Once the site has a real domain, set `NEXT_PUBLIC_SITE_URL=https://yourdomain.com` in your hosting provider's environment variables (or a local `.env.local` for testing) so OpenGraph links, the sitemap and `robots.txt` point at the right place.
